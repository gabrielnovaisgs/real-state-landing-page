...
export function Header() {
    return (
        <nav className="grid grid-cols-3 md:flex justify-between  font-sans items-center md:px-16 md:py-2 fixed top-0 left-0 right-0 z-40 bg-white shadow-md">
            <Image src={"/logo-title.svg"} alt="logo" width={200} height={100}
                className="h-16 col-start-2 "
            />
            <ul className="hidden md:flex gap-4 text-base items-center uppercase">
                <LinkMenu>Home</LinkMenu>
                <LinkMenu>Propriedades</LinkMenu>
                <LinkMenu>Agentes</LinkMenu>
                <LinkMenu>Contato</LinkMenu>
                <li className="text-primary text-base px-4 py-2 border border-primary rounded-xl
